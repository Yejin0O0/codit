...
**예외 (R4, #74)**: 태그 선택 화면은 "추가"(직접 입력 확정)와 "저장"(화면 진행) 두 개의 `default` 버튼이 공존한다. "화면당 primary 1개" 규칙과 충돌하지만, "추가"는 진행 액션이 아니라 입력 확정 액션이라는 성격 차이로 개발자가 명시적으로 예외를 확정함 — 일반화하지 않고 이 화면만의 예외로 남긴다.
