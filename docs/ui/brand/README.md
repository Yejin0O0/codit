...
| `CollapsedTimer` pill (Shadow DOM, ~18px) | 마크 (C + 체크) | **인라인 SVG** (`CoditMark withCheck`), `currentColor` / `text-primary` | 로고 트레이스, 그림자·베벨·배경 제거 | ✅ 굵은 C + 민트 체크 |
...
- **`CoditMark` SVG 는 로고의 굵은 C + 민트 체크를 평면으로 재현한다** (팔각 디테일·배경 배지 생략 — pill/헤더 크기에서 안 읽힘). path 좌표 SoT = `components/codit/codit-mark.tsx`.
