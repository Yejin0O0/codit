...
| `BrandHeader` (Extension Page 상단 — auth / history) | 마크 + "Codit" | **인라인 SVG** (`CoditMark withCheck`) + 그라데이션 텍스트 (워드마크) | 로고 트레이스 | ✅ 굵은 C 획 + 민트 체크 |
| `CollapsedTimer` pill (Shadow DOM, ~18px) | 마크 (C 단색) | **인라인 SVG** (`CoditMark`), `currentColor` / `text-primary` | 로고 트레이스, 그림자·베벨·배경 제거 | ✅ 굵은 C 단색 (완료 시 pill 이 별도 체크 아이콘) |
| 펼친 위젯 헤더 (`PanelShell`) | 마크 (C 단색) | **인라인 SVG** (`CoditMark`), `text-primary` | 위와 동일 | ⬜ UI-L2 R0 (#52) 에서 추가 |
...
- **제품 UI 안(BrandHeader·pill·위젯 헤더)은 래스터 PNG를 쓰지 않는다.** Shadow DOM 격리 + "인라인 SVG only" 아키텍처 규칙([`../ui-architecture.md`](../ui-architecture.md)). 렌더된 로고의 3D 베벨·그림자·그라데이션은 16px에서 뭉개진다 → 평면 인라인 SVG로 트레이스한다.
- **`CoditMark` SVG 는 로고의 굵은 C(+ 옵션 민트 체크)를 평면으로 재현한다** (팔각 디테일·배경 배지 생략 — pill/헤더 크기에서 안 읽힘). 체크는 `withCheck` 로 켜며 풀 락업(`BrandHeader`)만 사용, pill·위젯 헤더는 단색 C. path 좌표 SoT = `components/codit/codit-mark.tsx`.
